...
- **What that choice costs:** with no directory to anchor identity, the config has *two* independent selectors — `terraform workspace select` (state + `Environment` tag) and `-var-file` (resource names + sizing) — which can silently disagree. This is not theoretical: during the chapter, an `errored.tfstate` was left holding `chapter-4-dev-*` resources while `.terraform/environment` read `prod`. See Chapter 4 guide §3.
...
- **Entry criteria:** a GitHub repo exists for this project — this chapter's whole point is PR-triggered `plan` and merge-triggered `apply`, so there needs to be somewhere for that to happen.
- **What the pipeline deploys (decided — this chapter's own small stack, not Chapter 4's):** `chapter-5-pipeline/stack/` is a single versioned, private, encrypted S3 bucket, and the pipeline deploys only that. The chapter is about the *pipeline*; the mechanics are identical regardless of what sits underneath, so the thing underneath should be the cheapest and fastest option available. Three reasons, in order of weight:
  - **Cost.** "Apply on merge to `main`" against Chapter 4's stack means every merge starts billing a NAT Gateway and an RDS instance — roughly $0.25/hr with both environments up. An empty S3 bucket costs nothing, so a pipeline that fires on every merge stays free.
  - **Iteration.** Getting OIDC working took roughly a dozen pushes (trust policy claim format, wrong role ARN, environment claim mismatch). Each attempt against Chapter 4's stack would be a ~10 minute apply; against a bucket it is seconds.
  - **Leak surface.** Chapter 4's stack generates a database password, and this chapter posts plan output into pull request comments. A stack with no secrets in it is what makes that safe — see the Chapter 5 guide §4.
- **Consequence, accepted:** Chapter 4's environments are never automated. The workflow transfers unchanged if that is ever wanted, and the earlier concern about CI needing to assert `terraform workspace show` before applying is moot here — `stack/` has a single default workspace, so there is nothing to select wrongly.
- **Exit criteria:** a PR against this repo produces a posted `terraform plan` automatically; a merge to `main` triggers `terraform apply` automatically; plan and apply use **separate IAM roles**, with the plan role scoped so unreviewed pull request code cannot write; no long-lived AWS access keys stored anywhere (GitHub secrets, local files) — auth is OIDC role assumption only.
- **Status — complete.** Both workflows proven against the real repo: a plan comment posted by `github-actions` on PR #7, and three successful merge-triggered applies. `bootstrap/` (OIDC provider + two roles) is applied by hand and matches its code with no drift; `stack/` is applied only by CI. The plan role's policy names every resource explicitly and can reach neither the other chapters' state nor `bootstrap/`'s, so the pipeline cannot widen its own permissions.
- **Cost checkpoint:** effectively zero — the IAM OIDC provider and roles are free, and an empty versioned S3 bucket costs nothing. This is still the point where accidental applies become easy to trigger, so be deliberate about what reaches `main`.
- **Open decisions:** none. OIDC-based auth is fixed by the README. The GitHub Environment approval gate was tried and removed: with a single deploy target and one person merging, it added a second claim format to keep in sync for no real review value, and the pull request already provides the review. Note for any future revival — declaring `environment:` in a job rewrites the token's `sub` claim, so the trust policy must change at the same time.
...
| What the CI pipeline deploys | Chapter 5 (decided) | Its own single S3 bucket, **not** Chapter 4's stack — apply-on-merge against a NAT Gateway and RDS would bill on every merge, and a stack holding a DB password cannot have its plan posted to a PR |
| CI authentication | Chapter 5 (decided) | GitHub OIDC, two roles — read-only from pull requests, write only from `main`. No long-lived keys anywhere |
| Deploy approval gate | Chapter 5 (decided) | None. The pull request is the review; a GitHub Environment gate was tried and removed as ceremony for a single-target, single-person project |
